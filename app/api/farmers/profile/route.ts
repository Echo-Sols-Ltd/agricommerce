...
const PROFILE_PATHS = ['/farmers/profile', '/farmers/me', '/users/me', '/auth/profile'];
...
export async function GET(request: NextRequest) {
  ...
  for (const path of PROFILE_PATHS) {
    try {
      const response = await tryFetchProfile(backendBaseUrl, path, headers);
      ...
      console.error(`Farmer profile proxy error for path ${path}:`, error);
    }
  }
  ...
  if (lastResponse) {
    ...
    const message =
      errorBody && errorBody.length < 500
        ? errorBody
        : 'Failed to fetch farmer profile from the backend service.';
  ...
  return NextResponse.json(
    {
      success: false,
      message: 'Unable to reach the farmer profile service.',
    },
    { status: 502 }
  );
}
